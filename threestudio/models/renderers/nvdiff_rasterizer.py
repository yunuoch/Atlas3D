...
class NVDiffRasterizer(Rasterizer):
    ...
    def render_with_transformation(self,
                                   R,
                                   t,
                                   t_mask,
                                   mvp_mtx: Float[Tensor, "B 4 4"],
        ...
        # triangle mask (remove triangles that are not visible)
        if t_mask is not None:
            mesh.t_pos_idx = mesh.t_pos_idx[t_mask]

        # Transform
        mesh.v_pos = torch.mm(mesh.v_pos, R.T) + t

        v_pos_clip: Float[Tensor, "B Nv 4"] = self.ctx.vertex_transform(
            mesh.v_pos, mvp_mtx)
        rast, _ = self.ctx.rasterize(v_pos_clip, mesh.t_pos_idx,
                                     (height, width))
        mask = rast[..., 3:] > 0
        mask_aa = self.ctx.antialias(mask.float(), rast, v_pos_clip,
                                     mesh.t_pos_idx)

        out = {"opacity": mask_aa, "mesh": mesh}

        gb_normal, _ = self.ctx.interpolate_one(mesh.v_nrm, rast,
                                                mesh.t_pos_idx)
        gb_normal = F.normalize(gb_normal, dim=-1)
        gb_normal_aa = torch.lerp(torch.zeros_like(gb_normal),
                                  (gb_normal + 1.0) / 2.0, mask.float())
        gb_normal_aa = self.ctx.antialias(gb_normal_aa, rast, v_pos_clip,
                                          mesh.t_pos_idx)
        out.update({"comp_normal": gb_normal_aa})  # in [0, 1]
        ...
        if render_rgb:
            ...
            gb_pos, _ = self.ctx.interpolate_one(mesh.v_pos, rast,
                                                 mesh.t_pos_idx)
            gb_viewdirs = F.normalize(gb_pos -
            ...
            positions = gb_pos[selector]
            # (Needed for correct material mapping)
            positions = torch.mm(positions - t, R)
            geo_out = self.geometry(positions, output_normal=False)
            ...
            gb_rgb_bg = self.background(dirs=gb_viewdirs)
            gb_rgb = torch.lerp(gb_rgb_bg, gb_rgb_fg, mask.float())
            gb_rgb_aa = self.ctx.antialias(gb_rgb, rast, v_pos_clip,
                                           mesh.t_pos_idx)
